...
이 문서는 Coffee Order System이 구현해야 할 목표 HTTP 계약입니다. 현재 저장소는 Spring Boot 부트스트랩 단계이므로 아래 엔드포인트가 이미 동작한다는 의미가 아닙니다. 구현 시 이 문서의 필드명·HTTP 상태·응답 코드를 임의로 바꾸지 않습니다.
...
수신 시스템은 이 저장소에서 구현하지 않으며 통합 테스트에서는 Mock HTTP API로 대체합니다. base URL은 기본값 없는 `COLLECTION_API_BASE_URL` 환경 변수로 주입하며 현재 범위에서는 인증 헤더를 전송하지 않습니다.
...
6. 워커는 기본 1초마다 `next_retry_at`, `created_at`, `event_id` 오름차순으로 최대 50건을 선점합니다.
...
| 연결·응답 timeout | 재시도 가능 | 다음 시도 예약 또는 마지막 시도 후 `FAILED` |
...
총 시도 횟수는 최초 1회를 포함해 4회입니다. 어느 시도에서든 재시도하지 않는 `4xx`가 오면 즉시 `FAILED`가 됩니다.
