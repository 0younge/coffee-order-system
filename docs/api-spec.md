...
이 문서는 Coffee Order System이 구현해야 할 목표 HTTP 계약입니다. 현재 저장소는 공통 응답 경계와 메뉴 목록 API까지 구현됐고 포인트·주문·인기 메뉴 API는 구현 중입니다. 구현 시 이 문서의 필드명·HTTP 상태·응답 코드를 임의로 바꾸지 않습니다.
