...
이 문서는 Coffee Order System이 구현해야 할 목표 HTTP 계약입니다. 현재 저장소는 애플리케이션·MySQL·Flyway 기반선까지 구현됐지만 아래 업무 엔드포인트는 아직 구현 중입니다. 구현 시 이 문서의 필드명·HTTP 상태·응답 코드를 임의로 바꾸지 않습니다.
