...
| 스키마 관리 | Flyway | 스키마와 초기 메뉴 데이터의 버전 관리 |
| 인증 | BCrypt, JWT Access Token | 비밀번호 해시와 무상태 인증 |
...
| 변경 API 멱등성 | [ADR-0002: 충전과 주문에 멱등키 적용](./adr/0002-protect-mutations-with-idempotency-keys.md) |
...
| 포인트 모델 | [ADR-0007: 원장 없이 현재 포인트 잔액 저장](./adr/0007-store-current-point-balance-without-ledger.md) |
...
- 정량 성능 목표는 임의로 정하지 않는다. 먼저 실제 부하 조건과 지표를 측정한 뒤 합의한다.
...
JWT는 HS256으로 서명하고 최소 32바이트 비밀키를 환경 변수로 주입한다. 필수 claim은 `sub` 사용자 ID, `iat`, `exp`, `iss=coffee-order-system`이다. `jti`, `aud`, `kid`와 애플리케이션 내부 키 회전 기능은 현재 범위에 포함하지 않는다.
...
2. DB 트랜잭션을 시작하고 `PROCESSING` 멱등 레코드를 선점한다.
3. 이미 같은 키가 있으면 저장된 `request_hash`와 현재 요청을 비교한다.
...
예상 가능한 비즈니스 실패에는 잔액 부족, 메뉴 부재와 보유 잔액 상한 초과가 포함된다. 인증 실패, 멱등키 누락·형식 오류, JSON 오류, 필수 필드 누락과 잘못된 충전 금액은 트랜잭션 시작 전에 반환하고 멱등 결과로 저장하지 않는다.
...
충전과 주문은 멱등키 선점 후 사용자 행을 잠그는 일관된 순서를 사용한다. 트랜잭션 안에서 외부 HTTP 호출이나 불필요한 계산을 하지 않아 동일 사용자의 락 대기 시간을 줄인다.
...
인증·헤더·JSON과 충전 금액 1~1,000,000P 검증은 트랜잭션 전에 끝낸다. 충전 트랜잭션은 다음 순서로 처리한다.
...
DB의 `point_balance BETWEEN 0 AND 10000000` CHECK는 애플리케이션 검증을 보조한다. 신규 사용자는 0P로 시작하고 한 번의 충전은 최대 1,000,000P다. 충전 후 잔액 상한을 넘으면 `409 POINT_BALANCE_LIMIT_EXCEEDED`를 멱등 결과로 저장한다.
...
`claim_token`은 UUID 문자열이다. 워커는 기본 1초마다 `next_retry_at`, `created_at`, `event_id` 오름차순으로 최대 50건을 선점한다. 폴링 주기와 배치 크기는 환경 설정으로 바꿀 수 있다.
...
|---:|---|---|
| 1 | 주문 커밋 후 가능한 즉시 | `retry_count=1`, 1분 뒤 예약 |
...
HTTP 연결 timeout은 2초, 읽기 timeout은 5초, 이벤트 lease는 30초다. DNS·TLS를 포함한 timeout 적용 범위는 HTTP 클라이언트 선택 시 검증하고 redirect는 비활성화한다.

외부 API의 base URL은 기본값 없는 `COLLECTION_API_BASE_URL` 환경 변수로 주입한다. 현재 범위에서는 인증 헤더를 사용하지 않고 `POST /events/orders`의 응답 본문도 성공 판정이나 저장에 사용하지 않는다.
...
현재 확정된 정량 SLO나 기준 부하는 없다. 따라서 임의의 응답 시간이나 RPS를 목표로 선언하지 않고 다음 항목을 측정 가능하게 만든다.
...
- readiness는 DB 연결과 필수 마이그레이션 상태를, liveness는 프로세스 생존을 구분해 관찰한다. 정확한 endpoint와 판정 기준은 배포 환경 확정 후 정한다.
- `FAILED` 이벤트, 오래된 `PENDING`, deadlock, 락 timeout, DB 풀 고갈은 알림 후보이며 수치 임계값은 성능 기준과 함께 합의한다.
- JWT 서명 키와 DB 자격 증명은 환경 변수 또는 배포 환경의 비밀 저장소로 주입하고 저장소에 커밋하지 않는다. 외부 API 주소는 `COLLECTION_API_BASE_URL` 환경 변수로 주입한다.
