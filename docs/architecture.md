...
이 문서는 사용자가 선택한 커피 주문 시스템의 **목표 아키텍처**를 설명한다. ADR별 승인 여부는 [ADR 목록](./adr/)에서 관리한다. 저장소에는 Spring Boot·MySQL·Flyway 기반선과 Menu·Point·Order·Idempotency·Outbox 모듈, 관측 경계가 구현됐으며 독립 애플리케이션 Context의 공용 DB 처리까지 검증됐다. 코드, Flyway 마이그레이션, [ERD](./erd.md), [API 명세](./api-spec.md)의 일치는 전체 검증에서 함께 확인한다.
...
| 보일러플레이트 | Lombok (`compileOnly`, `annotationProcessor`) | 생성자 주입·로거·보호된 JPA 기본 생성자의 제한적 생성 |
...
Outbox 워커를 활성화해 `bootRun`할 때는 기본값이 없는 `COLLECTION_API_BASE_URL`을 실행 환경에 설정해야 한다.

Compose와 CI는 MySQL `8.4` 이미지 태그를 사용한다. JDBC 연결 풀과 Flyway 자동 구성, V1~V3 migration이 구현돼 있으며 기준선 변경은 새 결정으로 관리한다.

현재 구현 단계에서 추가가 승인된 애플리케이션 의존성은 `spring-boot-starter-validation`, `spring-boot-starter-actuator`, Spring Boot 4의 Flyway 자동 구성 모듈인 `org.springframework.boot:spring-boot-flyway`, `org.flywaydb:flyway-core`, `org.flywaydb:flyway-mysql`과 제한적으로 사용하는 Lombok이다. Lombok은 `compileOnly`와 `annotationProcessor`로만 선언하며 런타임 의존성으로 두지 않는다. 빌드 도구로는 Spotless 플러그인과 Java 포매터가 승인됐다. 외부 Mock API, 비동기 HTTP와 동시성 테스트는 JDK 표준 기능으로 구현하며 그 밖의 라이브러리는 추가 전에 다시 확인한다.

통합 테스트는 같은 Compose MySQL 인스턴스의 `coffee_order_system_test`를 사용해 개발 데이터베이스와 분리한다. Compose는 멱등적인 `docker/mysql/init/01-create-test-database.sh`를 MySQL 초기화 디렉터리에 연결해 신규 볼륨의 테스트 DB와 권한을 자동 준비한다. 기존 볼륨은 같은 스크립트를 컨테이너 안에서 명시적으로 한 번 실행하며 볼륨과 개발 데이터는 삭제하지 않는다. 일반 테스트에서는 Outbox 워커를 비활성화하고 Outbox 전용 테스트에서만 Mock HTTP 서버와 함께 활성화한다. 다중 인스턴스 테스트는 서로 다른 랜덤 포트의 독립 Spring Context 두 개가 같은 테스트 DB를 사용한다.
...
| 사용자·멱등 잠금 순서 | [ADR-0025: 외래 키 교착을 피하도록 사용자 행을 멱등 레코드보다 먼저 잠금](./adr/0025-lock-user-before-idempotency-record.md) |
...
| 기능 내부 계층과 Facade | [ADR-0026: 기능 우선 계층형 아키텍처와 공개 Facade 경계](./adr/0026-refine-feature-first-layered-architecture.md) |
| Lombok 사용 | [ADR-0027: 제한된 Lombok 정책](./adr/0027-use-lombok-with-restrictions.md) |
...
| Outbox 필드 수명주기 | [ADR-0023: Outbox 상태별 필드 수명주기 확정](./adr/0023-define-outbox-field-lifecycle.md) |
| 과제·로컬 기준 사용자 | [ADR-0024: 과제와 로컬 실행용 기준 사용자 seed](./adr/0024-seed-reference-user-for-local-execution.md) |
...
- 원문의 실시간 전송 요구는 주문 응답과 외부 장애를 분리한 준실시간 Outbox로 구현하고 정상·backlog 없음 조건의 최초 HTTP 요청 시작을 커밋 후 2초 이내로 제한한다.
...
| Application | Facade, 유스케이스, Command, Result, 기능 간 공개 계약, 트랜잭션 조정 | Spring MVC 타입, 다른 기능의 API·Domain·Infrastructure 직접 참조 |
| Domain | Entity, Value Object, 불변식, 계산 규칙, 상태 전이 | API·Application·Infrastructure, Spring MVC, Jackson, JdbcTemplate |
| Infrastructure | JPA·JdbcTemplate 저장소, 외부 HTTP Client, Scheduler, Configuration | 다른 기능이 직접 import하는 공개 협력 경로 |
...
`response_body`는 MySQL JSON 컬럼이므로 저장·조회 과정에서 객체 키 순서와 공백이 바뀔 수 있고, 그 순서는 MySQL 버전을 가로지르는 공개 계약이 아니다. 전용 JSON codec이 최초 응답 작성과 DB 조회 모두에서 객체 키를 재귀적으로 사전순 정렬하고 배열 순서는 유지해, DB의 내부 표현에 기대지 않으면서 최초·저장·재사용 HTTP status와 JSON 필드·값·null을 동일하게 유지한다.
...
| 포인트 충전 | 사용자 행 잠금, 멱등 레코드, 잔액 증가, 결과 저장 | **모두 한 트랜잭션** | 없음 |
| 주문 및 결제 | 사용자 행 잠금, 멱등 레코드, 메뉴 조회, 잔액 차감, 주문·Outbox·결과 저장 | **모두 한 트랜잭션** | 없음 |
...
- Flyway는 과제·로컬 수동 검증용 기준 사용자 `id=1`, `point_balance=0`을 준비한다. 자동화 테스트는 이 행에 의존하지 않고 독립 fixture를 사용한다.
...
1. `userId`, 헤더 존재 여부와 기본 요청 형식을 검증한다.
2. DB 트랜잭션을 시작하고 사용자 행을 PK `FOR UPDATE`로 조회한다. 사용자가 없으면 멱등 레코드 없이 거절한다.
3. `INSERT ... ON DUPLICATE KEY UPDATE id = id`로 멱등키를 선점한 뒤 해당 레코드를 잠금 조회한다.
4. 이미 완료된 키면 저장된 `request_hash`와 현재 요청을 비교한다.
5. 해시가 같고 상태가 `COMPLETED`면 저장된 HTTP 상태와 응답 본문을 그대로 재사용한다.
6. 해시가 다르면 업무 변경 없이 `409 IDEMPOTENCY_KEY_REUSED`를 반환한다.
7. 최초 요청이면 같은 트랜잭션에서 충전 또는 주문을 처리한다.
8. 성공 또는 예상 가능한 비즈니스 실패 결과를 멱등 레코드에 `COMPLETED`로 저장하고 커밋한다.
9. DB 연결 실패처럼 예상하지 못한 인프라 오류는 멱등 레코드를 포함해 전체 롤백한다.

> API 오류 코드의 정식 표기는 [API 명세](./api-spec.md)를 따른다. 위 6번의 응답 코드는 `IDEMPOTENCY_KEY_REUSED`다.
...
충전과 주문은 사용자 행 잠금 후 멱등 레코드를 잠그는 일관된 순서를 사용한다. 멱등 레코드의 사용자 외래 키를 확인할 때 부모 행 공유 락을 먼저 획득하면 배타 락 승격 과정에서 교착할 수 있으므로 사용자 행을 먼저 잠근다. 모든 DB 커넥션에는 `SET SESSION innodb_lock_wait_timeout=5`를 적용한다. 트랜잭션 안에서 외부 HTTP 호출이나 불필요한 계산을 하지 않아 동일 사용자의 락 대기 시간을 줄인다.
...
`userId`·헤더·JSON과 signed `BIGINT` 범위의 양의 정수 충전금액을 먼저 검증한다. 사용자는 충전 트랜잭션의 첫 `FOR UPDATE` 조회로 확인하고, 없으면 멱등 처리 전에 거절한다. 트랜잭션은 다음 순서로 처리한다.

1. 요청한 사용자 행을 PK로 `FOR UPDATE` 조회한다.
2. 멱등 레코드를 선점하거나 완료 결과를 재사용한다.
...
HTTP 헤더·JSON·`userId`·`menuId` 형식을 먼저 검증한다. 기존 사용자는 아래 트랜잭션의 첫 `FOR UPDATE` 조회로 확인하며, 없으면 멱등 처리 전에 거절한다. 복합 오류의 공개 우선순위는 [API 명세](./api-spec.md#입력-검증과-오류-우선순위)를 따른다.
...
sequenceDiagram
    ...
    API->>DB: 트랜잭션 시작 및 사용자 행 FOR UPDATE
    API->>DB: 기존 사용자 확인 및 멱등키 선점
    API->>DB: 메뉴 조회
...
1. 요청한 사용자 행을 PK로 `FOR UPDATE` 조회하고 사용자가 없으면 거절한다.
2. 멱등 레코드를 선점하거나 완료 결과를 재사용한다.
3. `menus`에서 요청한 메뉴의 현재 이름과 가격을 읽는다.
...
`retry_count`는 현재 `claim_token`으로 결과 반영에 성공한 실패 시도 횟수를 뜻한다. 재시도 가능 여부와 관계없이 반영된 실패마다 1 증가하고, fencing으로 거절된 이전 claim의 결과는 세지 않는다. 자동 재시도 상태 전이의 논리적 전송 시도는 최대 4회지만, lease 만료와 응답 유실로 생기는 at-least-once 중복 HTTP 요청은 이 상한 밖에서 추가될 수 있다.
...
재시도 가능한 실패 뒤 `PENDING`과 이를 선점한 `PROCESSING`은 직전 실패의 `last_http_status`, `last_error_type`을 보존한다. HTTP 응답이 없는 네트워크 오류와 timeout의 `last_http_status`는 `NULL`이다. `PUBLISHED` 전환은 `next_retry_at`, claim 정보와 마지막 오류 정보를 모두 `NULL`로 정리하며, 영구 오류 또는 네 번째 실패의 `FAILED` 전환은 `next_retry_at`과 claim 정보를 `NULL`로 정리하고 마지막 오류 정보를 보존한다.
...
외부 API의 base URL은 기본값 없는 `COLLECTION_API_BASE_URL` 환경 변수로 주입한다. HTTP(S) URL의 명시 포트는 `1~65535`만 허용하고 기존 base path를 보존해 그 뒤에 `events/orders`를 결합한다. Outbox 워커가 활성화된 경우 누락되거나 잘못되면 애플리케이션 시작이 실패한다. 현재 범위에서는 별도 자격 증명 헤더를 사용하지 않고 외부 요청의 응답 본문도 성공 판정이나 저장에 사용하지 않는다.

### 12.4 상태 보존

- `PUBLISHED` 이벤트는 발행 완료 후 최소 30일간 보존하고 이후 삭제할 수 있다.
- `FAILED` 이벤트에는 증가한 `retry_count`, `failed_at`, 선택적인 `last_http_status`, 필수 `last_error_type`을 저장하고 자동 삭제하지 않는다.
- 응답 본문과 stack trace는 DB에 저장하지 않는다.
- 자동 정리 스케줄러와 수동 재처리 API·운영 화면은 현재 범위가 아니다.

## 13. 인기 메뉴 조회

기준 시각 `to` 직전 7×24시간인 `[to - 7일, to)` 구간에서 `PAID` 주문을 직접 집계한다. 주문 시각은 MySQL `DATETIME(6)`에 저장되므로 `to`가 더 세밀하면 두 경계를 다음 microsecond로 올림해 저장 가능한 시각 집합에서 같은 반개구간을 표현한다. 주문 수 내림차순, 동률이면 메뉴 ID 오름차순으로 정렬하고 최대 3개를 반환한다.
...
`PT-*` 부하·성능 기준선은 별도 후속 작업으로 실행한다. 현재 완료 게이트에는 기능·동시성·외부 계약·품질 검사와 정상 조건의 Outbox 최초 요청 2초 검증까지만 포함한다.
...
- 요청 ID, 사용자 ID, 주문 ID, 이벤트 ID를 key-value 구조화 로그의 연관 키로 사용한다. 외부 JSON 로그 라이브러리는 추가하지 않는다.
- 멱등키 전체 값과 DB 자격 증명은 로그에 남기지 않는다.
- 외부 호출 로그에는 대상, 시도 번호, 지연, 결과 분류, 이벤트 ID를 남기되 payload의 민감 정보는 최소화한다.
- Spring MVC의 `http.server.requests`와 HikariCP 기본 지표로 API 요청 수·지연·오류와 DB 풀 상태를 측정한다.
- 사용자 정의 지표는 DB lock timeout·deadlock 카운터, Outbox 전송 결과·재시도·최종 실패·fencing 거절 카운터, `PENDING`·`FAILED` 건수와 가장 오래된 대기 이벤트 시간 gauge로 제한한다. 사용자·주문·이벤트 ID처럼 cardinality가 증가하는 값을 metric tag로 사용하지 않는다.
- HTTP에는 상세 정보를 숨긴 `/actuator/health`만 노출한다. 지표 endpoint, 외부 exporter, JSON 로그 라이브러리와 알림 연동은 배포 환경 확정 후 정한다.
