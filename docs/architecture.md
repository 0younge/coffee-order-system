...
| 인증 | Spring Security, BCrypt, JWT Access Token | 비밀번호 해시와 무상태 인증 |
...
`bootRun` 전에 기본값이 없는 `JWT_SECRET_BASE64`와 `COLLECTION_API_BASE_URL`을 실행 환경에 설정해야 한다.

Compose는 MySQL `8.4` 이미지 태그를 사용한다. JDBC 연결 풀 설정과 Flyway 파일 구성은 구현 시 정하되 위 기준선을 변경하지 않는다.

현재 구현 단계에서 추가가 승인된 애플리케이션 의존성은 `spring-boot-starter-validation`, `spring-boot-starter-oauth2-resource-server`, `spring-boot-starter-actuator`, `org.flywaydb:flyway-core`, `org.flywaydb:flyway-mysql`이다. 빌드 도구로는 Spotless 플러그인과 Java 포매터가 승인됐다. 외부 Mock API, 비동기 HTTP와 동시성 테스트는 JDK 표준 기능으로 구현하며 그 밖의 라이브러리는 추가 전에 다시 확인한다.

통합 테스트는 같은 Compose MySQL 인스턴스의 `coffee_order_system_test`를 사용해 개발 데이터베이스와 분리한다. 일반 테스트에서는 Outbox 워커를 비활성화하고 Outbox 전용 테스트에서만 Mock HTTP 서버와 함께 활성화한다. 다중 인스턴스 테스트는 서로 다른 랜덤 포트의 독립 Spring Context 두 개가 같은 테스트 DB와 JWT 키를 사용한다.
...
| 인증 | [ADR-0008: 무상태 JWT Access Token 인증](./adr/0008-use-stateless-jwt-access-token-authentication.md) |
...
| JWT 구현 | [ADR-0012: JWT 구현에 Spring Security 사용](./adr/0012-use-spring-security-for-jwt.md) |
...
| REST 보안 경계 | [ADR-0021: 무상태 Bearer REST 보안 경계 구성](./adr/0021-configure-stateless-bearer-security-boundary.md) |
...
| 기능 | 책임 | 주 데이터 | 트랜잭션 역할 |
|---|---|---|---|
| Auth | 회원가입, BCrypt 검증, JWT 발급·검증 | `users` 인증 컬럼 | 가입 쓰기, 로그인 읽기 |
| Menu | 고정 메뉴 목록, 인기 메뉴 응답 조합 | `menus`, 집계된 `orders` | 읽기 |
| Point | 인증 사용자의 현재 잔액 충전 | `users.point_balance` | 사용자 행 잠금 후 증가 |
...
|---|---|---|---|
| 회원가입 | 이메일 중복 확인, `users` 삽입 | 사용자 생성 한 트랜잭션 | 없음 |
| 로그인 | 이메일로 사용자 조회, BCrypt 검증 | 읽기 경계 | 없음 |
...
- 로그인 성공 시 30분 동안 유효한 JWT Access Token 하나만 발급한다.
- 보호 API는 `Authorization: Bearer <access-token>`의 검증된 사용자 ID를 인증 주체로 사용한다.
- 요청 본문의 사용자 ID를 신뢰하지 않는다.
- 원문 요구사항의 사용자 식별값은 JWT `sub`에 대응한다.
- 서버는 인증 세션을 저장하지 않으므로 어느 인스턴스나 요청을 처리할 수 있다.
- Refresh Token, 로그아웃 토큰 폐기 목록, OAuth는 이번 범위에 포함하지 않는다.
- 비밀번호 원문과 JWT 전체 문자열은 로그에 남기지 않는다.
- 서버 인증 세션을 만들지 않고 CSRF, form login과 HTTP Basic을 비활성화한다.
- 회원가입·로그인·메뉴 목록·인기 메뉴와 상세 정보를 숨긴 health endpoint는 공개하며 충전·주문은 JWT 인증을 요구한다.
- 프론트엔드가 범위 밖이므로 CORS 설정을 추가하지 않는다.

JWT 발급·검증과 Bearer 인증은 Spring Security OAuth2 Resource Server와 Nimbus를 사용한다. 서명 키는 `JWT_SECRET_BASE64`로 주입하며 Base64 디코딩 후 최소 32바이트여야 한다. 기본값은 없고 누락되거나 잘못되면 애플리케이션 시작이 실패한다. 필수 claim은 `sub` 사용자 ID의 10진 문자열, `iat`, `exp`, `iss=coffee-order-system`이며 만료 허용 오차는 0초다. `jti`, `aud`, `kid`와 애플리케이션 내부 키 회전 기능은 현재 범위에 포함하지 않는다. 토큰 검증을 위해 사용자 DB를 매번 조회하지 않는다.
...
예상 가능한 비즈니스 실패에는 잔액 부족, 메뉴 부재와 포인트 덧셈 overflow가 포함된다. 인증 실패, 멱등키 누락·형식 오류, JSON 오류, 필수 필드 누락과 잘못된 충전 금액은 트랜잭션 시작 전에 반환하고 멱등 결과로 저장하지 않는다.
...
인증·헤더·JSON과 signed `BIGINT` 범위의 양의 정수 충전금액 검증은 트랜잭션 전에 끝낸다. 충전 트랜잭션은 다음 순서로 처리한다.

1. 멱등 레코드를 선점하거나 완료 결과를 재사용한다.
2. 인증 사용자 행을 PK로 `FOR UPDATE` 조회한다.
...
3. 인증 사용자 행을 PK로 `FOR UPDATE` 조회한다.
...
외부 API의 base URL은 기본값 없는 `COLLECTION_API_BASE_URL` 환경 변수로 주입한다. 누락되거나 잘못되면 애플리케이션 시작이 실패한다. 현재 범위에서는 인증 헤더를 사용하지 않고 `POST /events/orders`의 응답 본문도 성공 판정이나 저장에 사용하지 않는다.
...
|---|---|---|---|
| 로그인 | 이메일 단건 조회 | `users(email)` UNIQUE | 인덱스 단건 접근, BCrypt 처리 시간 |
...
- 멱등키 전체 값, 비밀번호, 비밀번호 해시, JWT 원문, DB 자격 증명은 로그에 남기지 않는다.
