...
현재 구현 단계에서 추가가 승인된 의존성은 `spring-boot-starter-validation`, `spring-boot-starter-oauth2-resource-server`, `spring-boot-starter-actuator`, `org.flywaydb:flyway-core`, `org.flywaydb:flyway-mysql`이다. 외부 Mock API, 비동기 HTTP와 동시성 테스트는 JDK 표준 기능으로 구현하며 그 밖의 라이브러리는 추가 전에 다시 확인한다.
