...
이 문서는 사용자가 선택한 요구사항을 데이터 구조로 구체화한 **논리 모델 정본**이다. ADR별 승인 여부는 [ADR 목록](./adr/)에서 관리한다. 아래 테이블·제약·기준 데이터는 Flyway로 구현됐으며, 업무 API와 트랜잭션은 아직 구현 중이다. 변경 시 Flyway 마이그레이션과 실제 MySQL 8.4 LTS 스키마를 이 문서와 대조한다.
