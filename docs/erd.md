...
이 문서는 사용자가 선택한 요구사항을 데이터 구조로 구체화한 **목표 논리 모델 초안**이다. ADR별 승인 여부는 [ADR 목록](./adr/)에서 관리하며, 저장소에는 애플리케이션 부트스트랩만 있다. 아래 테이블이 모두 구현됐다는 뜻은 아니다. 구현 시 Flyway 마이그레이션과 실제 MySQL 8.4 LTS 스키마를 이 문서와 대조한다.

- 금액과 포인트는 소수점 오차가 없는 정수로 표현하며 `1원 = 1P`다.
- 날짜·시간은 UTC로 저장하고 API 경계에서 ISO 8601로 표현한다.
- 문자열은 `utf8mb4`를 사용하고 이메일은 대소문자를 무시하는 `utf8mb4_0900_ai_ci` collation으로 비교한다.
- 업무 ID는 `BIGINT AUTO_INCREMENT`, 이벤트 ID와 claim token은 UUID 문자열 `CHAR(36)`, 시각은 UTC `DATETIME(6)`을 사용한다.
- Mermaid 도식의 `varchar_254`, `char_36`, `datetime_6` 표기는 각각 SQL의 `VARCHAR(254)`, `CHAR(36)`, `DATETIME(6)`을 뜻한다.
- 상태에 따라 존재하지 않는 값으로 명시한 컬럼 외에는 모두 `NOT NULL`이다.
...
|---|---|---|---|
| `users` → `orders` | 사용자 1명당 주문 0개 이상 | 주문은 존재하는 사용자 ID를 참조 | 인증 사용자만 자신의 주문 생성 |
...
인증 정보와 현재 사용할 수 있는 포인트 잔액을 보유한다. 포인트 원장이나 충전·사용 이력 테이블은 이번 범위에 두지 않는다.

| 컬럼 | 의미 | 논리 규칙 |
|---|---|---|
| `id` | 사용자 식별자 | PK, 주문·멱등 레코드의 참조 대상 |
| `email` | 로그인 식별 이메일 | UNIQUE, 가입 시 중복 불가 |
| `password_hash` | BCrypt 결과 | 원문 저장 금지 |
| `point_balance` | 현재 포인트 잔액 | `NOT NULL DEFAULT 0`, `0` 이상 `10,000,000` 이하 |
| `created_at` | 생성 시각 | UTC |
| `updated_at` | 사용자 정보 또는 잔액 변경 시각 | UTC |

충전과 주문은 `id`로 사용자 행을 `FOR UPDATE` 조회한다. 같은 사용자의 변경은 직렬화되고 서로 다른 사용자는 서로 다른 행을 잠근다. 애플리케이션 검증과 별도로 `CHECK (point_balance BETWEEN 0 AND 10000000)`를 둔다. 한 번의 충전 요청은 최대 1,000,000P다.

이메일은 앞뒤 공백을 제거한 값을 저장하되 입력 대소문자는 유지한다. `VARCHAR(254)`와 대소문자를 무시하는 collation의 UNIQUE 제약으로 대소문자만 다른 가입도 중복으로 처리한다.
...
| `retry_count` | 완료된 실패 시도 횟수 | 최초 `0`, 최댓값 `4` |
| `next_retry_at` | 다음 전송 가능 시각 | 최초 즉시, 재시도 시 1·5·30분 반영 |
...
| `last_http_status` | 마지막 HTTP 상태 | 네트워크·timeout이면 `NULL` |
| `last_error_type` | 마지막 실패 분류 | `TIMEOUT`, `NETWORK`, `HTTP_3XX`, `HTTP_4XX`, `HTTP_5XX` |
...
`event_id`와 `claim_token`은 UUID `CHAR(36)`이다. 완료 뒤 `locked_at`과 `claim_token`은 `NULL`로 정리한다. `PUBLISHED`에서는 `published_at`, `FAILED`에서는 `failed_at`과 `last_error_type`이 `NOT NULL`이다. 응답 본문과 stack trace는 저장하지 않는다.
...
|---|---|---|---|
| `users` | `id` | PK | 사용자 식별 |
| `users` | `email` | UNIQUE | 중복 가입 보조 방지 |
| `users` | `point_balance BETWEEN 0 AND 10000000` | CHECK | 잔액 범위 방지 |
| `menus` | `id` | PK | 메뉴 식별 |
...
| `users` | `(email)` | 예 | 가입 중복 방지, 로그인 | collation 결과, 단건 접근 |
...
HTTP `2xx`는 `PUBLISHED`다. 네트워크 오류·timeout·`408`·`429`·`5xx`는 남은 시도가 있으면 재시도한다. redirect는 따르지 않으며 `3xx`와 그 밖의 `4xx`는 즉시 `FAILED`다.

## 8. Outbox 점유와 fencing

다음 SQL은 물리 DDL이 아니라 트랜잭션 의도를 보여주는 개념 예시다. 기본 `batch_size`는 50이며 워커는 1초마다 실행한다.
...
같은 짧은 트랜잭션에서 선택 행을 `PROCESSING`으로 바꾸고 새 `claim_token`, `locked_at`을 기록한다. 외부 HTTP 호출은 커밋 뒤 수행한다. 결과 갱신은 다음 조건을 포함한다.
...
영향받은 행이 0개면 다른 워커가 재점유한 것이므로 이전 결과를 버린다. fencing은 DB 상태 덮어쓰기는 막지만 외부 API 중복 호출은 막지 못하므로 수신 측은 `event_id`로 중복을 제거한다. 연결 timeout은 2초, 읽기 timeout은 5초, lease는 30초다.
...
- [ADR-0002: 충전과 주문에 멱등키 적용](./adr/0002-protect-mutations-with-idempotency-keys.md)
...
- [ADR-0007: 원장 없이 현재 포인트 잔액 저장](./adr/0007-store-current-point-balance-without-ledger.md)
- [ADR-0008: 무상태 JWT Access Token 인증](./adr/0008-use-stateless-jwt-access-token-authentication.md)
