...
`PENDING`은 claim 정보와 완료 시각이 없고, `PROCESSING`은 claim 정보가 있으며, `PUBLISHED`와 `FAILED`는 각 완료 시각이 존재하고 claim 정보가 없다는 조합을 MySQL CHECK로 강제한다.
...
| `outbox_events` | `retry_count BETWEEN 0 AND 4` | CHECK | 완료된 실패 시도 횟수 범위 |
| `outbox_events` | `status IN ('PENDING', 'PROCESSING', 'PUBLISHED', 'FAILED')` | CHECK | 전달 상태 제한 |
| `outbox_events` | 상태별 claim·완료 시각·실패 유형 NULL 조합 | CHECK | 점유와 완료 상태의 모순 방지 |
