...
2026-07-17 현재 애플리케이션 기반선, MySQL 스키마, 메뉴 조회, 포인트 충전, 주문·결제, 인기 메뉴 조회, Outbox 전달, 관측성·엄격한 API 계약과 다중 인스턴스 교차 경계는 구현·검증됐다. 기능 우선 계층형 구조와 제한된 Lombok 정책도 승인된 패키지 이동·구조 테스트·빌드 반영과 전체 회귀로 구현·검증됐다. 아래 테스트 ID 중 실행 증거에 연결되지 않은 항목은 아직 **계획 식별자**이며, 테스트 소스와 통과 결과가 생기기 전까지 검증 증거가 아니다.
...
| 검증 자동화 | 검증됨 | 테스트 DB·워커 격리, Spotless `check` 연결, MySQL·Mock HTTP·독립 웹 Context 기반 전체 117개 테스트와 문서 링크·ID·현재 범위 상태 검사; [#1 증거](#기반선-검증-증거-github-1), [#2 증거](#메뉴-api-검증-증거-github-2), [#3 증거](#포인트-충전-검증-증거-github-3), [#4 증거](#주문결제-검증-증거-github-4), [#5 증거](#인기-메뉴-검증-증거-github-5), [#6 증거](#outbox-전달-검증-증거-github-6), [#7 증거](#관측성과-api-계약-검증-증거-github-7), [#8 증거](#다중-인스턴스와-추적성-검증-증거-github-8), [#9 증거](#최종-통합-검증-증거-github-9), [#12 증거](#outbox-설정-경계-검증-증거-github-12), [#13 증거](#outbox-워커-복구와-배치-선점-검증-증거-github-13), [#14 증거](#http-오류-응답-검증-증거-github-14), [#15 증거](#주문-시각-정밀도-검증-증거-github-15), [#16 증거](#fresh-mysql-ci-검증-증거-github-16), [#17 증거](#문서와-하네스-동기화-검증-증거-github-17), [#18 증거](#최종-전체-리뷰-검증-증거-github-18), [#30 증거](#주문-계층-경계-증거-github-30), [#31 증거](#outbox-전달-계층-경계-증거-github-31), [#32 증거](#common-api오류-경계-증거-github-32), [#33 증거](#전체-구조-검증-증거-github-33), [#37 증거](#optional구조-테스트-유지보수-검증-증거-github-37) | 없음 |
...
4. 재현 커밋: `816a0ac`(전체 소스 구조·우회 검증 완성), `278b217`(구조 문서·추적성 동기화), `4c4540b`(모든 Controller 형태와 인스턴스 의존성 우회 차단), `2b2c21a`(package-private·annotation·완전 수식 필드 탐지), `341d8b2`(`@Controller`의 DB 직접 접근 차단); GitHub 작업 이슈 `#33`

### Optional·구조 테스트 유지보수 검증 증거 (GitHub #37)

1. 구현: [OrderFacade.java](../src/main/java/com/example/coffeeordersystem/order/application/OrderFacade.java)가 메뉴 조회 `Optional`의 empty를 명시적으로 분기하면서 `completeFailure(MENU_NOT_FOUND)` 멱등 완료 경로를 유지하고, [LayeredArchitectureTest.java](../src/test/java/com/example/coffeeordersystem/quality/LayeredArchitectureTest.java)는 Java Unicode escape를 먼저 해석한 의존성 분석용 소스에서만 주석·문자열·문자·text block을 제거한다. SQL·Outbox·Lombok 원문 검사는 그대로 유지한다.
2. 테스트: 메뉴 부재 결과 저장·상태 변경 뒤 최초 응답 재사용·주문과 Outbox 무부수효과를 검증하는 `AT-ORDER-002`·`AT-CONTRACT-001`·`AT-CONTRACT-003` — [OrderApiTest.java](../src/test/java/com/example/coffeeordersystem/order/OrderApiTest.java); 사용자 락 이후 결제 시각 확정을 검증하는 `CT-ORDER-001` — [OrderFacadeTest.java](../src/test/java/com/example/coffeeordersystem/order/application/OrderFacadeTest.java); 원문 오탐 제거와 실제 import·static import·완전 수식·계층 위반 탐지를 검증하는 `QT-ARCH-001` — [LayeredArchitectureTest.java](../src/test/java/com/example/coffeeordersystem/quality/LayeredArchitectureTest.java)
3. 실행: 2026-07-17 MySQL 8.4 healthy에서 주문 관련 테스트 13개와 구조 테스트 7개가 각각 성공했다. `./gradlew clean check build` 성공; 전체 117개, 성공 117개, 실패·오류·제외 0개. 각 변경에서 `./gradlew bootRun` 시작과 `GET /actuator/health`의 `UP`을 확인했고, 독립 리뷰의 최종 must·should·nit은 모두 0개다.
4. 재현 커밋·CI: `64275f9`(Optional empty 명시 분기, GitHub `#35`, [CI](https://github.com/0younge/coffee-order-system/actions/runs/29524343206)), `ba9f398`(구조 검사 소스 정규화와 우회 회귀 테스트, GitHub `#36`, [CI](https://github.com/0younge/coffee-order-system/actions/runs/29525657493)); 두 실행 모두 fresh MySQL에서 성공했다.
...
| `POL-ARCH-01` | 기능 우선 API·Application·Domain·Infrastructure 책임, Controller→Facade, 다른 기능은 공개 Application 계약만 참조하고 순환·빈 위임 계층 금지 | [모듈러 모놀리스](./architecture.md#6-모듈러-모놀리스), [기능 경계](./context.md#기능-경계와-의존-방향) | [0006](./adr/0006-use-feature-oriented-modular-monolith.md), [0026](./adr/0026-refine-feature-first-layered-architecture.md) | `QT-ARCH-001`, 전체 `AT-*`·`IT-*`·`CT-*` 회귀 | 검증됨 | 변경 전 103개 회귀 기준은 [#20 증거](#구조-리팩토링-기준선-증거-github-20)로 고정했고, 기능별 패키지·Facade 이동은 `#26`~`#32`, 순환·Controller·계층 경계와 115개 전체 회귀는 [#33 증거](#전체-구조-검증-증거-github-33), 원문 오탐 제거와 117개 전체 회귀는 [#37 증거](#optional구조-테스트-유지보수-검증-증거-github-37)로 검증 |
| `POL-LOMBOK-01` | Lombok은 `compileOnly`·`annotationProcessor`의 유일한 신규 의존성으로 사용하고 생성자·로거·보호된 JPA 기본 생성자·선택 Getter에 제한하며 위험한 Entity annotation 금지 | [승인된 기술 기준선](./architecture.md#2-승인된-기술-기준선), [테스트 환경](./test-strategy.md#41-환경-계약) | [0027](./adr/0027-use-lombok-with-restrictions.md) | `QT-DEPS-001`, `QT-LOMBOK-001` | 검증됨 | `#22`에서 Gradle scope와 유일 신규 의존성, `#23`·`#24`에서 Spring 생성자·로거와 Entity 제한 annotation, `#25`와 [#33 증거](#전체-구조-검증-증거-github-33)에서 annotation processing·금지 정책·전체 빌드를 검증 |
| `POL-IDEM-01` | 사용자 행 잠금 뒤 MySQL upsert·잠금 조회로 키를 선점하고 업무 처리와 멱등 결과를 한 트랜잭션에 커밋 | [멱등 처리](./architecture.md#9-멱등-처리), [ERD](./erd.md#44-idempotency_records) | [0015](./adr/0015-protect-mutations-with-idempotency-keys.md), [0025](./adr/0025-lock-user-before-idempotency-record.md) | `IT-ORDER-001`, `IT-ORDER-002`, `IT-ORDER-003`, `IT-IDEM-001`, `IT-IDEM-002`, `CT-IDEM-001`, `CT-IDEM-002`, `QT-HARNESS-001` | 검증됨 | 충전의 선점·결과 재사용·키 충돌은 [#3 증거](#포인트-충전-검증-증거-github-3), 주문의 성공·실패 원자 커밋과 동시 재시도는 [#4 증거](#주문결제-검증-증거-github-4), 메뉴 부재 실패의 저장·재사용 회귀는 [#37 증거](#optional구조-테스트-유지보수-검증-증거-github-37), 문서·하네스의 잠금 선행 안내는 [#17 증거](#문서와-하네스-동기화-검증-증거-github-17)로 검증 |
