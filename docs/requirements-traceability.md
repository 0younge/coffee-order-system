...
2026-07-16 현재 애플리케이션 기반선, MySQL 스키마, 메뉴 조회, 포인트 충전, 주문·결제와 인기 메뉴 조회는 구현·검증됐고 Outbox 전달은 구현 중이다. 아래 테스트 ID 중 실행 증거에 연결되지 않은 항목은 아직 **계획 식별자**이며, 테스트 소스와 통과 결과가 생기기 전까지 검증 증거가 아니다.
...
| 애플리케이션 시작점 | 검증됨 | [CoffeeOrderSystemApplication.java](../src/main/java/com/example/coffeeordersystem/CoffeeOrderSystemApplication.java), UTC `Clock`, health 최소 공개; [#1 증거](#기반선-검증-증거-github-1) | Outbox 전달 워커 |
| Spring context·설정 테스트 | 검증됨 | 인기 메뉴까지 포함한 MySQL 기반 전체 63개 테스트; [#1 증거](#기반선-검증-증거-github-1), [#2 증거](#메뉴-api-검증-증거-github-2), [#3 증거](#포인트-충전-검증-증거-github-3), [#4 증거](#주문결제-검증-증거-github-4), [#5 증거](#인기-메뉴-검증-증거-github-5) | 후속 업무 기능별 테스트 |
| 기능 구현 | 구현 중 | 공통 HTTP 응답 경계, Menu·Popular Menu 조회, Point 충전·Idempotency, Order 결제·Outbox 원자 저장 구현·검증 완료 | Outbox 전달 워커 |
| 데이터베이스 | 검증됨 | [V1 migration](../src/main/resources/db/migration/V1__create_schema_and_seed_reference_data.sql), [V2 migration](../src/main/resources/db/migration/V2__make_lifecycle_codes_case_sensitive.sql), [DatabaseMigrationTest.java](../src/test/java/com/example/coffeeordersystem/database/DatabaseMigrationTest.java); [#1 증거](#기반선-검증-증거-github-1) | Outbox 전달 상태 쿼리 |
| 검증 자동화 | 검증됨 | 테스트 DB·워커 격리, Spotless `check` 연결, MySQL 기반 전체 63개 테스트; [#1 증거](#기반선-검증-증거-github-1), [#2 증거](#메뉴-api-검증-증거-github-2), [#3 증거](#포인트-충전-검증-증거-github-3), [#4 증거](#주문결제-검증-증거-github-4), [#5 증거](#인기-메뉴-검증-증거-github-5) | 후속 이슈의 계층별 업무·동시성·외부 연동 테스트와 CI 증거 |
