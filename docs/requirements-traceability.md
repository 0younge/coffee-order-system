...
| 기능 우선 계층·Lombok 리팩토링 | 구현 중 | [ADR-0026](./adr/0026-refine-feature-first-layered-architecture.md), [ADR-0027](./adr/0027-use-lombok-with-restrictions.md), GitHub 추적 이슈 `#19`, [#20 기준선](#구조-리팩토링-기준선-증거-github-20); Lombok 빌드·소스와 구조 테스트 `#22`~`#25`, Idempotency·Menu·Point 계층 경계 `#26`~`#28` | Order·Outbox·Common 계층 이동, 전체 구조 테스트와 최종 실행 증거 |
...
2. 테스트: 승인된 현재 범위의 전체 `UT-*`·`IT-*`·`AT-*`·`EXT-*`·`CT-*`·`QT-*` — [전체 테스트 소스](../src/test/java/com/example/coffeeordersystem), [TraceabilityTest.java](../src/test/java/com/example/coffeeordersystem/quality/TraceabilityTest.java), [OrderServiceTest.java](../src/test/java/com/example/coffeeordersystem/order/OrderServiceTest.java), [PointFacadeTest.java](../src/test/java/com/example/coffeeordersystem/point/application/PointFacadeTest.java)
...
1. 구현: JSON 응답 협상을 핸들러 실행 전에 확정하는 [메뉴](../src/main/java/com/example/coffeeordersystem/menu/api/MenuController.java)·[포인트](../src/main/java/com/example/coffeeordersystem/point/api/PointController.java)·[주문](../src/main/java/com/example/coffeeordersystem/order/OrderController.java) 매핑, DB 시간 정밀도의 [인기 메뉴 구간](../src/main/java/com/example/coffeeordersystem/menu/domain/PopularMenuWindow.java), 엄격한 [Outbox 워커 설정](../src/main/java/com/example/coffeeordersystem/outbox/OutboxWorkerConfiguration.java), [Claude autodev 포인터](../.claude/skills/autodev), 하네스 변경도 테스트를 다시 실행하게 하는 [Gradle 외부 입력](../build.gradle)
