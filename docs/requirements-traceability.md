...
2026-07-16 현재 애플리케이션 기반선과 MySQL 스키마는 구현·검증됐고 업무 기능은 구현 중이다. 아래 테스트 ID 중 [기반선 검증 증거](#기반선-검증-증거-github-1)에 연결되지 않은 항목은 아직 **계획 식별자**이며, 테스트 소스와 통과 결과가 생기기 전까지 검증 증거가 아니다.
...
| Spring context·설정 테스트 | 검증됨 | 기반선과 메뉴 API를 포함한 MySQL 기반 전체 22개 테스트; [#1 증거](#기반선-검증-증거-github-1), [#2 증거](#메뉴-api-검증-증거-github-2) | 후속 업무 기능별 테스트 |
| 기능 구현 | 구현 중 | 공통 HTTP 응답 경계와 Menu 조회 구현·검증 완료 | Point, Order, Idempotency, Popular Menu, Outbox 업무 코드 |
...
| 검증 자동화 | 검증됨 | 테스트 DB·워커 격리, Spotless `check` 연결, MySQL 기반 전체 22개 테스트; [#1 증거](#기반선-검증-증거-github-1), [#2 증거](#메뉴-api-검증-증거-github-2) | 후속 이슈의 계층별 업무·동시성·외부 연동 테스트와 CI 증거 |
