...
| `POL-OUTBOX-06` | 워커 정상·전송 가능한 기존 backlog 없음 조건에서 주문 커밋 후 2초 이내 최초 HTTP 요청 시작 | [Outbox 전달](./architecture.md#12-outbox-전달), [외부 API](./api-spec.md#12-외부-데이터-수집-api-계약) | [0017](./adr/0017-bound-first-outbox-attempt-latency.md) | `EXT-OUTBOX-002`, `EXT-OUTBOX-008`, `PT-OUTBOX-001` | 미구현 | 없음 |
| `POL-AUTH-01` | Spring Security와 Nimbus로 JWT를 처리하고 `JWT_SECRET_BASE64` 누락·오류 시 시작 실패 | [인증 경계](./architecture.md#8-인증-경계) | [0012](./adr/0012-use-spring-security-for-jwt.md) | `UT-AUTH-002`, `UT-AUTH-003`, `AT-AUTH-005`, `QT-CONFIG-001` | 미구현 | 없음 |
...
| `POL-PERF-01` | Outbox 정상 최초 요청 2초 기준 외 목표치는 `TBD`; p50/p95/p99, 처리량, lock wait, 실행 계획과 backlog를 먼저 측정 | [테스트 전략](./test-strategy.md#56-성능-기준선-테스트) | [0017](./adr/0017-bound-first-outbox-attempt-latency.md) | `EXT-OUTBOX-008`, `PT-API-001`, `PT-LOCK-001`, `PT-POPULAR-001`, `PT-OUTBOX-001` | 측정 미실시 | 2초 계약 외 수치 기준과 실행 결과 없음 |
...
| `POL-TEST-01` | 개발·테스트가 단일 Compose MySQL을 사용하되 테스트 소유 데이터만 정리하고 전체 초기화는 금지 | [데이터 격리](./test-strategy.md#7-데이터-격리와-재현성) | [0010](./adr/0010-test-against-mysql-with-docker-compose.md) | 전체 `IT-*`, `EXT-*`, `CT-*` | 미구현 | 테스트 데이터 격리 코드 없음 |
| `POL-DEPS-01` | 신규 의존성은 Validation·OAuth2 Resource Server·Actuator·`org.flywaydb:flyway-core`·`org.flywaydb:flyway-mysql`만 승인하고 Mock·비동기·polling은 JDK 사용 | [승인된 기술 기준선](./architecture.md#2-승인된-기술-기준선), [테스트 환경](./test-strategy.md#41-환경-계약) | [0005](./adr/0005-establish-java-spring-mysql-platform-baseline.md), [0012](./adr/0012-use-spring-security-for-jwt.md), [0013](./adr/0013-use-actuator-and-micrometer-for-observability.md) | `QT-DEPS-001`, `QT-CONFIG-001` | 미구현 | 현재 build에는 승인 의존성 추가 전 |
