...
> **문서와 구현 상태**
>
> 이 저장소의 문서는 목표 계약을 설명합니다. ADR별 승인 여부는 [ADR 목록](./docs/adr/)에서 관리합니다. 현재 코드는 Spring Boot·MySQL·Flyway 기반선과 메뉴 목록 API까지 구현됐습니다. 포인트·주문·인기 메뉴 API와 외부 연동 워커는 아직 구현되지 않았으므로 문서의 목표 동작 전체를 구현 완료 상태로 해석하지 않습니다.
