...
> **문서와 구현 상태**
>
> 이 저장소의 문서는 목표 계약을 설명합니다. ADR별 승인 여부는 [ADR 목록](./docs/adr/)에서 관리합니다. 현재 코드는 Spring Boot·MySQL·Flyway 기반선, 메뉴·포인트·주문·인기 메뉴 API, Outbox 분산 전달 워커, 관측 로그·지표를 구현했습니다. 실제 MySQL을 공유하는 독립 애플리케이션 인스턴스 두 개의 교차 처리까지 검증하며, 예약된 정리 기능과 합의되지 않은 성능 기준선은 현재 범위에서 제외합니다.
...
| 흐름 | 처리 요약 | 정합성 경계 |
|---|---|---|
| 포인트 충전 | 사용자 행 잠금 → 멱등 요청 확인 → 덧셈 overflow 검사 → 잔액 증가 → 최초 결과 저장 | 비즈니스 변경과 멱등 레코드를 하나의 MySQL 트랜잭션으로 커밋 |
| 주문 및 결제 | 사용자 행 잠금 → 멱등 요청 확인 → 현재 메뉴 조회 → 잔액 차감 → 주문 스냅샷과 Outbox 저장 | 포인트·주문·Outbox·멱등 결과를 하나의 MySQL 트랜잭션으로 커밋 |
