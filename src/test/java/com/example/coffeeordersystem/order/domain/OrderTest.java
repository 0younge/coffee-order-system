...
class OrderTest {
  ...
  @Test
  @DisplayName("UT-ORDER-002 DB에서 허용하는 스냅샷으로 결제 완료 주문을 만든다")
  void createsPaidOrder() {
    assertNotNull(Order.paid(1L, 2L, "메뉴", 4_000L, PAID_AT));
    assertNotNull(Order.paid(1L, 2L, " ", 4_000L, PAID_AT));
    assertNotNull(Order.paid(1L, 2L, "😀".repeat(100), 4_000L, PAID_AT));
  }
